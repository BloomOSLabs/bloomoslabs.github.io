...
export default function TeamCard ({ member, maintainedDevices }: TeamCardProps) {
  const [imageError, setImageError] = useState(false)

  const showAvatar = !!member.avatar && !imageError
  ...
  return (
    <CardLayout align='center'>
      {showAvatar ? (
        ...
        <a className='logo' aria-hidden='true'>
          <svg
      ...
      {maintainedDevices.length > 0 && (
        <div className='team-devices'>
          <div className='team-devices-title'>Maintains</div>

          <div className='team-device-chips'>
            {maintainedDevices.map(device => (
              <DeviceChip key={device.codename}>{device.name}</DeviceChip>
            ))}
          </div>
        </div>
      )}
